...
#[builder(state_mod(name = oauth2_form_request_builder))]
...
pub enum SerializeOAuth2FormError<DPoPErr: crate::Error> {
    ...
    SerializeForm {
        /// The underlying error from `serde_html_form`.
        source: serde_html_form::ser::Error,
    },
    /// There was an error when attempting to serialize the authentication parameters.
    #[snafu(display("Failed to serialize authentication parameters"))]
    SerializeAuth {
        /// The underlying error from `serde_html_form`.
        source: serde_html_form::ser::Error,
    },
...
pub enum HandleResponseError {
    ...
    #[snafu(display("Failed to parse error response as OAuth2 error"))]
    ...
    #[snafu(display("OAuth2 request failed with an OAuth2 error payload"))]
    OAuth2 {
